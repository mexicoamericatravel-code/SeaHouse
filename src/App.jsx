    ...
    <header className="sticky top-0 z-40 border-b border-white/70 bg-white/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 py-4 lg:px-8">
        ...
        <nav className="hidden items-center gap-6 text-sm font-medium md:flex">
          ...
          <a href="#location" className="text-slate-600 hover:text-slate-950">{t.navLocation}</a>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form name="booking-request" method="POST" data-netlify="true" className="mt-6 space-y-4">
              ...
              <select
                ...
                className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-700 outline-none focus:border-slate-400"
              >
...
function SimplePageCard({ page }) {
  const Icon = page.icon;
  return (
    <a href={page.href} className="group rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1">
      <div className="w-fit rounded-2xl bg-sky-50 p-3 text-sky-700"><Icon className="h-5 w-5" /></div>
      <h3 className="mt-4 text-xl font-semibold text-slate-950">{page.title}</h3>
      <p className="mt-3 text-sm leading-6 text-slate-600">{page.copy}</p>
      <p className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-slate-900">
        {page.cta} <ChevronRight className="h-4 w-4 transition group-hover:translate-x-1" />
      </p>
    </a>
  );
}
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <section className="bg-slate-50/70">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
          <SectionHeading eyebrow={t.pagesEyebrow} title={t.pagesTitle} />
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {siteData.subPages.map((page) => <SimplePageCard key={page.title} page={page} />)}
          </div>
        </div>
      </section>
